...
darknode up --do --name testing-do --network invalid-network
if [ "$?" -eq "0" ]; then
...
darknode up --do --name testing --do-token $do_token --do-region invalid-region
if [ "$?" -eq "0" ]; then
    exit "failed to pass test when providing invalid region"
fi

# Return error when providing an invalid digital ocean droplet type
darknode up --do --name testing --do-token $do_token --do-region invalid-region --do-droplet invalid-droplet-type
if [ "$?" -eq "0" ]; then
